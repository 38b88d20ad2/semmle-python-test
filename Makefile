...
lint: env/.requirements
	env/bin/flake8 VulnerablePythonLib tests
...
jenkins: env/.requirements
	env/bin/py.test -s --tb short --cov-config .coveragerc --cov VulnerablePythonLib --cov-report term-missing --cov-report xml \
	    --junitxml junit.xml \
	    tests

coverage: env/.requirements test
	env/bin/coverage run --source VulnerablePythonLib setup.py test
